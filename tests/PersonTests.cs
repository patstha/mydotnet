...
public class PersonTests
{
    ...
    public void ShortOrLongPasswordsFail(string name, string password)
    {
        ...
        Func<Person> func = () => PersonFactory.Create(name, password);
        func.Should().Throw<ArgumentNullException>().WithMessage($"The password provided to create user {name} is not valid. A password must have a minimum length no shorter than {MINIMUM_PASSWORD_LENGTH} and no longer than {MAXIMUM_PASSWORD_LENGTH}.");
    }
}
